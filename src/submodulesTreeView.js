...
class SubmodulesProvider {
	...
	getSubmodules() {
		return new Promise((resolve, reject) => {
			exec(
			    'git submodule status', {cwd: this.workspaceRoot},
			    (err, stdout, stderr) => {
				    if (err) {
					    vscode.window.showErrorMessage(
					        'Failed to retrieve submodules.');
					    return resolve([]);
				    }
				    ...
				    const submodules = stdout.trim().split('\n').map((line) => {
					    ...
					    // Obtenir le hash court (7 premiers caractères) ou "Not
					    // Initialized"
					    ...
					    if (parts[0].startsWith('-')) {
						    ...
						    icon =
						        new vscode.ThemeIcon('arrow-down');  // Behind
					    }
					    ...
					    return new SubmoduleItem(
					        submodulePath,  // Le nom du submodule est le label
					                        // principal
					        vscode.TreeItemCollapsibleState.None, {
						        command:
						            'zegit-submodules-manager.openSubmodule',
						        title: '',
						        arguments: [submodulePath],
					        },
					        icon,        // L'icône dépend du status
					        description  // Le commit hash ou "Not Initialized"
					                     // comme description
					    );
				    });
